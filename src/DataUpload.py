...
class dataUpload():
            ...
            for i2, file in enumerate(filelist):
                try:
                    ...
                    if any(x in file for x in ["meta.csv", "freq.csv", "data.csv"]):
                        remote_path = os.path.normpath(remoteBasePath + "/data/" + os.path.basename(folder) + "/" + file[0:4] + "/" + file[4:6] + "/" + file[6:8] + "/" + file)

                    elif any(x in file for x in ["data.tsv"]):
                        remote_path = os.path.normpath(remoteBasePath + "/data/" + os.path.basename(folder) + "/" + file[0:4] + "/" + file[4:6] + "/" + file)

                    elif any(x in file for x in ["snap.fits"]):
                        remote_path = os.path.normpath(remoteBasePath + "/" + os.path.basename(folder) + "/" + file[0:4] + "/" + file[4:6] + "/" + file[6:8] + "/" + file[8:10] + "/" + file)
    ...
    def UploadEvent(self, file, md5):
        ...
        print payload
        try:
            r = requests.get('http://vo.astro.cz/api/upload/%s/' %(self.value["project"]), params=payload)
            pass
        except Exception, e:
            print e
